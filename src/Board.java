...
public class Board extends JPanel
{
  ...
  private static final Color GRID_COLOR_C = new Color(47,171,196);
  private static final Color GRID_COLOR_D = new Color(70,100,158);
  ...
  private void paintText(Graphics g)
  {
    ...
    g.fillRect(x,5, this.getSize().width, (int)Math.round(GAP+FONT_SIZE*scale) );
    g.fillRect(x,y+15, this.getSize().width, (int)Math.round(GAP+FONT_SIZE*scale) );
    g.setColor( Color.black );
    g.drawString(message, x, y + (int)Math.round(FONT_SIZE*scale));
    g.drawString(messageTop, x, 5 + (int)Math.round(FONT_SIZE*scale));
    g.drawString(messageBelow, x, y+15 + (int)Math.round(FONT_SIZE*scale));
  }
  ...
  private void paintGrid(Graphics g)
  {
    for (int i = 0; i < this.grid.length; i++)
    {
      ...
      {    
        ...
        else if (((i%2 != 0 && j%2 == 0) || (i%2 == 0 && j%2 != 0))&& j>9){
  ...
  public void displayMessageTop(String theMessage)
  {
    messageTop = theMessage;
    this.repaint();
  }
  
  public void displayMessageBelow(String theMessage)
  {
    messageBelow = theMessage;
    this.repaint();
  }
